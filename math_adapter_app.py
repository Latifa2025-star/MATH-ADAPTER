...
.hero {
    background: radial-gradient(circle at top left, #fff7d6 0%, #e6f7ff 40%, #f5e6ff 75%, #e9fff1 100%);
...
def extract_local_count_plan(problem: str):
    ...
    for m in pattern.finditer(text):
        ...
        if obj in {
            "minute", "hour", "day", "week", "month", "year", "mile", "meter", "foot",
            "inch", "page", "more", "less", "left", "each", "every", "total", "amount", "group"
        }:
            continue
...
def build_operation_visual(problem: str, dataset_answer: str, plan: dict) -> str:
    ...
    nums = [word_to_num(x) for x in re.findall(
        r"\b(\d+|zero|one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve|thirteen|fourteen|fifteen|sixteen|seventeen|eighteen|nineteen|twenty|thirty|forty|fifty)\b",
        text
    )]
    nums = [x for x in nums if x is not None]

    # Addition
    if len(items) >= 2 and any(w in text for w in ["together", "altogether", "in all", "total", "combined", "sum"]):
        a = items[0]["count"]
        b = items[1]["count"]
        obj = answer_obj or items[0]["object"]
        color_a = items[0].get("color", "")
        color_b = items[1].get("color", "")
        return f"""
        {block(items[0]['object'], color_a, a, f'{a} {items[0]["object"]}(s)')}
        <div class="eqline">+</div>
        {block(items[1]['object'], color_b, b, f'{b} {items[1]["object"]}(s)')}
        <div class="eqline">=</div>
        {block(obj, "", final_num, f'{final_num} {obj}(s)')}
        """

    # Subtraction
    if any(w in text for w in ["left", "remain", "after", "gave away", "lost", "spent", "used", "fewer", "less"]):
        if len(nums) >= 2:
    ...
    # Multiplication
    if any(w in text for w in ["each", "every", "times", "double", "twice"]):
        if len(nums) >= 2:
            a, b = nums[0], nums[1]
            obj = answer_obj or (items[0]["object"] if items else "item")
            color = items[0].get("color", "") if items else ""
            return f"""
            <div class='smalltext'><b>{a} groups</b></div>
            <div class="eqline">×</div>
            <div class='smalltext'><b>{b} in each group</b></div>
            <div class="eqline">=</div>
            {block(obj, color, final_num, f'{final_num} {obj}(s)')}
            """

    # Division
    if any(w in text for w in ["split", "share equally", "equally", "divide", "among"]):
        if len(nums) >= 2:
            a, b = nums[0], nums[1]
            obj = answer_obj or (items[0]["object"] if items else "item")
            color = items[0].get("color", "") if items else ""
...
def render_full(problem, dataset_answer, mode, bundle=None, story_img_html="", exact_html="", operation_html=""):
    st.markdown(
        card_start("badge-orig", "📖 Original Problem (Full)")
        + f"<div class='bigtext'>{problem}</div>"
        + f"<div class='smalltext'><b>Correct answer (dataset):</b> {dataset_answer}</div>"
        + card_end(),
        unsafe_allow_html=True,
    )
    ...
    if mode == "Adapter + emojis + illustration":
        ...
        with c1:
            st.markdown(
                card_start("badge-img", "🖼️ Story Illustration")
                + story_img_html
                + card_end(),
                unsafe_allow_html=True,
            )
        with c2:
            st.markdown(
                card_start("badge-exact", "🎯 Exact Count Visual")
                + exact_html
                + card_end(),
                unsafe_allow_html=True,
            )
        ...
        st.markdown(
            card_start("badge-op", "🧮 Operation Visual")
            + operation_html
            + card_end(),
            unsafe_allow_html=True,
        )
    ...
    if bundle:
        ...
        with c1:
            st.markdown(
                card_start("badge-adhd", "⚡ ADHD Version")
                + f"<div class='bigtext'>{bundle.get('ADHD_problem', '')}</div>"
                + card_end(),
                unsafe_allow_html=True,
            )
        with c2:
            st.markdown(
                card_start("badge-ell", "🟢 ELL Version")
                + f"<div class='bigtext'>{bundle.get('ELL_problem', '')}</div>"
                + card_end(),
                unsafe_allow_html=True,
            )
        with c3:
            st.markdown(
                card_start("badge-id", "🧠 ID Version")
                + f"<div class='bigtext'>{bundle.get('ID_problem', '')}</div>"
                + card_end(),
                unsafe_allow_html=True,
            )

        st.markdown(
            card_start("badge-sol", "👩‍🏫 Teacher Solution")
            + f"<div class='bigtext'>{bundle.get('teacher_solution', '')}</div>"
            + card_end(),
            unsafe_allow_html=True,
        )
        ...
        with c1:
            st.markdown(
                card_start("badge-sol", "⚡ ADHD Explanation")
                + f"<div class='bigtext'>{bundle.get('ADHD_expl', '')}</div>"
                + card_end(),
                unsafe_allow_html=True,
            )
        with c2:
            st.markdown(
                card_start("badge-sol", "🟢 ELL Explanation")
                + f"<div class='bigtext'>{bundle.get('ELL_expl', '')}</div>"
                + card_end(),
                unsafe_allow_html=True,
            )
        with c3:
            st.markdown(
                card_start("badge-sol", "🧠 ID Explanation")
                + f"<div class='bigtext'>{bundle.get('ID_expl', '')}</div>"
                + card_end(),
                unsafe_allow_html=True,
            )
...
if xml_bytes is None:
    st.markdown(
        "<div class='hero'><h1>🌈 Math Problem Adapter</h1><p>Upload <b>ASDiv.xml</b> in the sidebar to begin.</p></div>",
        unsafe_allow_html=True,
    )
    st.stop()
...
if only_with_color:
    df = df[
        df["problem_text_full"].str.lower().str.contains(
            r"\b(red|green|blue|yellow|orange|purple|pink|brown|black|white)\b",
            regex=True,
            na=False,
        )
    ]
...
<div class="hero">
  ...
  <p>Pick a problem, generate learner-friendly rewrites, and show emoji math visuals with optional illustration.</p>
</div>
""", unsafe_allow_html=True)

problem_labels = [
    f"{i} — {re.sub(r'\\s+', ' ', str(t)).strip()}"
    for i, t in enumerate(df["problem_text_full"].tolist())
]
...
problem = str(row["problem_text_full"])
dataset_answer = str(row["reference_answer"])
...
if not generate:
    ...
    render_full(
        ...
        mode=mode,
        bundle=None,
        story_img_html="<div class='smalltext'>Click Generate to create the illustration.</div>",
        exact_html=preview_exact,
        operation_html=preview_op,
    )
    st.stop()
